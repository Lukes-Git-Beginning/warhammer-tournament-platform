...
- **Drop = kein Auto-Forfeit (2026-06-19):** `POST /api/tournaments/:slug/participants/:userId/drop` setzt nur `participant.status = WITHDREW`. Laufende Matches (PENDING/ONGOING) bleiben unangetastet — Gegner kann normal reporten. Die frühere Forfeit-Logik (mit `matchGame.deleteMany` + `match.update FORFEIT/CANCELLED`) wurde entfernt. Swiss-Re-Pairing-Fix: `auto-swiss-service.ts` `generateNextSwissRound()` filtert jetzt `WITHDREW`-Spieler aus `swissPlayers` via `dbParticipants.status`-Check (vorher fehlte der Filter komplett → WITHDREW-Spieler wurden erneut gepairt).
