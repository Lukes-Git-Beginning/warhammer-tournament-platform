...
| `routes/matches.ts` / `routes/bracket.ts` / `routes/participants.ts` (erweitert, 2026-06-28 B-Batch) | **+B10:** `POST /api/matches/:id/no-contest` (technischer Double-Bye, setzt Match-Status `NO_CONTEST`). **+B12:** operative Match-/Bracket-Ops (swap-player, create-match, delete-match, forfeit, add-late, set-faction) sind jetzt **`canManageTournament`-gated** statt `ADMIN`-only — Host **und** Co-Hosts erreichen sie (globaler Admin behält stats/maps/factions/config/audit). **+B14:** spätere Swiss-Runden paaren nur `CHECKED_IN`. **+B13:** alle Tournament-Routen nutzen `host_id`/`host` (vormals `organizer_id`/`organizer`); die „ORGANIZER"-Spaltenwerte oben sind veraltet → faktisch `canManage` bzw. `HOST`/`ADMIN` | `authenticate` + `canManageTournament` |
...
| **`lib/playoff-generator.ts`** | **Welle 2 + B6 (2026-06-28)** — `generatePlayoffBracket()` für NONE/**TOP2**/TOP4/TOP8 mit kaskadierender Auto-Reduktion (TOP8→TOP4 <16, TOP4→TOP2 <8, TOP8→TOP2 <8); Zählbasis = aktive checked-in Spieler |
| **`lib/swiss.ts`** (B8, 2026-06-28) | Swiss-Pairing eigenständig: Minimum-Cost-Perfect-Matching via `edmonds-blossom` (gestaffelte Edge-Kosten no-contest≫rematch≫ΔScore²≫mirror, seeded shuffle); kein `tournament-pairings` mehr |
