...
- **2026-06-28 Batch (Branch `feat/2d3-mode`, additiv via psql + `migrate resolve`):**
  - **2D3-Modus:** `TournamentMode` Enum um `TWO_D_THREE` erweitert; `TournamentParticipant.faction_ids String[] @default([])` (die 3 bei Registrierung gewählten Fraktionen; pro Game zieht der Server zufällig eine, analog SFT-Latch).
  - **NO_CONTEST (B10):** `MatchStatus` Enum um `NO_CONTEST` erweitert (technischer Double-Bye, getrennt von DRAW). Standings: beide +1, 0 BH.
  - **host-Rename (B13) Migration `20260628020000_rename_organizer_to_host`:** `Tournament.organizer_id → host_id` (Postgres metadata-only `RENAME COLUMN` + Index + FK-Constraint, kein Datenrisiko). Relation `organizer → host`, `User.organized_tournaments → hosted_tournaments` (Relation `"HostedTournaments"`). Begründung: Rolle wurde bereits zu HOST vereinheitlicht (ORGANIZER-Purge, `remove_organizer_role`), das Feld zog jetzt nach.
...
- **2026-06-09 Migration `fix_duplicate_maps`**: Soft-deletes 4 stale map records whose slugs diverged from their names after admin renames. Affected slugs: `glades-of-the-everqueen`, `itza`, `rift-at-worlds-edge`, `skjlanadirs-cave`. Root cause: `POST /api/admin/maps/:id` PATCH updates `name` but not `slug` — if seed then re-runs it upserts by new slug and creates a duplicate. Gotcha: always re-run seed from scratch or invalidate slug manually after an admin rename.
- **2026-06-21 Migration `20260621000000_remove_organizer_role`**: `ORGANIZER` aus `Role`-Enum entfernt. Alle ORGANIZER-Accounts waren bereits per `20260612120000` zu HOST konvertiert, daher rein schematische Bereinigung. Postgres-Workaround: CREATE TYPE Role_new AS ENUM (...ohne ORGANIZER), ALTER TABLE User ALTER COLUMN role TYPE Role_new, DROP TYPE Role, RENAME.
- **2026-06-17 Migration `20260617100000_add_tournament_restricted_factions`**: Neues Model `TournamentRestrictedFaction (tournament_id, faction_id)` — mirrors `TournamentFactionAllowlist`. Games wo eine der Fraktionen restricted ist bekommen bei Finalisierung automatisch `counts_for_leaderboard = false`. Relation: `Tournament.restricted_factions TournamentRestrictedFaction[]`, `Faction.tournament_restrictions TournamentRestrictedFaction[]`. Backend: `tournaments.ts` POST/PATCH/GET unterstützt `restricted_factions: string[]`. Frontend: eigenes Fieldset in Create + Edit Forms (rotes Accent-Styling).
- **2026-06-14 Migration `add_match_counts_for_leaderboard`** (`20260614100000`): `Match.counts_for_leaderboard Boolean @default(true)` hinzugefügt. Ermöglicht per-Match Ausschluss vom Season-Leaderboard ohne Tournament-weiten Flag. `confirmedMatchWhere()` in `rating-model-service.ts` prüft jetzt Match-Level zusätzlich zu Tournament-Level. Admin-Endpoint: `PATCH /api/matches/:id/void`. Gotcha: Migration wurde manuell via Docker-psql applied + `_prisma_migrations`-Eintrag direkt inserted, weil Advisory-Lock durch Background-Prozess blockiert war.
