...
- [ ] **post-v1: `LeaderboardEntry.games_played/wins/losses` entfernen** — redundant zu MatchGame-Aggregation; Profil-Pfad (`users.ts`) ist seit 16.06. auf dynamische Aggregation migriert, Felder werden aber noch von `leaderboard.ts` (All-Time-Tab), `LeaderboardPage.tsx`, `UserProfilePage.tsx` gelesen → erst diese Rest-Lese-Stellen umstellen
...
- **16.06.** — A/B/C-Bugfix-Strang: **(A) Faction-Proficiency** als Win-Chance angezeigt (neues `neutralWinChance = logistic(skill)`, roher `playerFactionSkill` bleibt für die Sortierung). **(B) Profil↔Leaderboard-Konsistenz** — Profil-Stats `current_season`/`all_time` jetzt dynamisch aus `computeSeasonLeaderboard` (`getPlayerSeasonStats`/`getPlayerAllTimeStats`) statt aus `LeaderboardEntry`; Profil-`all_time` = Modell-Punkte (Float). **(C) Bracket-Display-Bugs** — leere GF/3rd-Slots nicht mehr als dropped/OUT (Frontend `MatchNode`), 3rd-Place-Verlierer-Slots „Loser of …" (Frontend `SVGBracket`); Backend-Härtung: `advance-playoffs`/`add-third-place-match` blockieren unplayed/cancelled Semifinals (422), `undrop` setzt drop-bedingte Playoff-Matches auf PENDING. Plus Doku-Cleanup + Replay-Format-Machbarkeitsstudie (Memory `project-replay-parsing`). `LeaderboardEntry`-Felder-Entfernung bleibt post-v1-Backlog.
- **16.06. (PM)** — Stats-Konsistenz & Open-Play-Hygiene (Branch `feat/game-stats-standings-openplay`): **Games statt Matches** als statistische Einheit — Admin-KPI „Matches Played" → „Games Played" (`MatchGame`-Count); **FactionStats-Recompute** (`lib/recompute-faction-stats.ts` + `POST /api/admin/recompute-faction-stats` + Dashboard-Button) baut Meta-Stats game-level aus `MatchGame` neu auf, idempotent, **behebt den FactionStats-Drift-Bug**. **Profil-Turnierzahl** zählt jetzt Teilnahme (≥1 gespieltes Game) statt finalisierter `TournamentResult` → gedroppte Spieler fehlen nicht mehr. **Standings W/D/L/B** (Draws explizit). **Stale-Open-Play-Cron** canceled hängende ONGOING-Ladder-Matches > 24 h. Anti-Farming bleibt absichtlich punkte-only (Faction Proficiency + Matchup nutzen weiter alle Games).

**Verbleibendes post-v1-Backlog (codebar, verifiziert offen):** 2FT/3FT-Formate (nur Frontend-Guard `SwissStandings.tsx:51` vorbereitet) · Liechtenstein Early Clinching + aktive Mirror-Vermeidung · MMR-Matchmaking-Format · `LeaderboardEntry`-Felder-Cleanup · `poster_url`-Upload-Flow (totes Schema-Feld) · **Auto-Replay-Verifikation (ESF-Parser)** — Replay-`.replay`-Format (ESF) ist analysiert: Fraktion/Map/Spieler/Armee per String-Extraktion trivial auslesbar; Sieger braucht echten ESF-Typcode-Walker (6 Ground-Truth-Dateien vorhanden). Nutzen: Anti-Cheat-Abgleich beim Upload bzw. Auto-Ergebnis im Open-Play-Ladder. Details: Memory `project-replay-parsing`. · **2×2 Ranked Matchup-Format** — jeder Spieler wählt 2 Fraktionen, dann werden alle 4 Matchups gerankt (Drag-and-Drop nach Präferenz), das System wählt das meistbeiderseits-gewünschte Matchup (Borda Count o.ä.); erfordert neues DB-Schema, Algorithmus, dnd-UI.
