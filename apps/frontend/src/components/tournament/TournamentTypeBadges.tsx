 ...
 * Renders the official image from `/battle-types/<type>.png` (dropped into public/battle-types/);
 * until that file exists it fails to load and renders nothing. Sits behind the content, so an
...
export function BattleTypeWatermark({ battleType }: { battleType?: string | null }) {
  ...
  if (!battleType || !BATTLE_TYPES.has(battleType) || failed) return null;
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden" aria-hidden>
      <img
        src={`/battle-types/${battleType.toLowerCase()}.png`}
        alt=""
