        ...
        return (
          <div key={entry.date} className="flex items-center gap-3 text-xs">
            ...
            <div className="flex-1 bg-stone-800 rounded-full h-2 overflow-hidden">
              <div
                className="h-full bg-warhammer-gold/60 rounded-full"
                style={{ width: `${barWidth}%` }}
              />
            </div>
            <span className="w-12 text-right text-stone-400">
              {entry.matches_played} Sp.
            </span>
    ...
    <main className="mx-auto max-w-4xl px-4 py-10">
      ...
      {stats ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5 mb-10">
          ...
          <StatCard label="Niederlagen" value={<span className="text-red-400">{stats.losses}</span>} />
          ...
          <StatCard
            ...
            value={
              stats.win_rate !== null ? (
                <span
                  className={
                    stats.win_rate >= 0.5 ? 'text-emerald-400' : 'text-red-400'
                  }
                >
