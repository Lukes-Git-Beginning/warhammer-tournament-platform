...
function LeaderboardTable({
  ...
  if (entries.length === 0) {
    return (
      <div className="py-8 text-center text-stone-500 text-sm">Noch keine Einträge.</div>
    );
  }
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-stone-800 bg-stone-900/60">
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-400">Spieler</th>
              ...
              <th className="px-4 py-3 text-right font-medium text-stone-400">Spiele</th>
              {extraColumn === 'seasons_participated' && (
                <th className="px-4 py-3 text-right font-medium text-stone-400">Seasons</th>
              )}
            </tr>
          </thead>
...
export function LeaderboardPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      <h1 className="font-display text-3xl font-bold text-warhammer-gold mb-6">Leaderboard</h1>

      {/* Tabs */}
      <div className="mb-6 flex gap-1 rounded-md border border-stone-800 bg-stone-900/60 p-1 w-fit">
        {(['season', 'all-time'] as const).map((tab) => (
          <button
            ...
            className={`rounded px-4 py-1.5 text-sm font-medium transition-colors ${
              activeTab === tab
                ? 'bg-warhammer-gold/20 text-warhammer-gold'
                : 'text-stone-400 hover:text-stone-200'
            }`}
          >
            {tab === 'season' ? 'Season' : 'All-Time'}
          </button>
        ))}
      </div>
